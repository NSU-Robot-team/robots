...
public class Main extends Application {
            ...
            for(int i=0; i<maxCommand;i++){
                for(int j=0; j<table.getRobotList().size();j++) {
                    if(pairs.get(j).comList.size()>i) {
                        currentlyX = pairs.get(j).ent.getX();
                        currentlyY = pairs.get(j).ent.getY();
                        if (checkClash(pairs.get(j), table, i) && pairs.get(j).comList.get(i).doCommand(pairs.get(j).ent)) {
                            ...
                            seqT.getChildren().addAll(trLinkedList.getLast(), new FillTransition(Duration.millis(30),
                                    table.getRectList().get(pairs.get(j).ent.getX()).get(pairs.get(j).ent.getY()).getCell() ,(Color) table.getRectList().get(pairs.get(j).ent.getX()).get(pairs.get(j).ent.getY()).getCell().getFill(), Color.RED));
                        }
                        else break;
                    }
    ...
    void makeStepAnimation(LinkedList<TranslateTransition> trLinkedList,Table table, Pair pairs,int i,int currentlyX,int currentlyY){

        trLinkedList.add(new TranslateTransition(javafx.util.Duration.millis(500),
                pairs.ent));

        table.getRectList().get(pairs.ent.getX()).get(pairs.ent.getY()).setCount(
                table.getRectList().get(pairs.ent.getX()).get(pairs.ent.getY()).getCount()+1);
